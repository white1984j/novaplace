<? include('header.php') ?>

			<div class="container no-gutters">
				<div class="row no-gutters">
					<div class="col content-sm d-none d-lg-flex">
						<? include('sidebar.php'); ?>
					</div>
					<div class="col content-bg b-content">
						<main class="b-detailed">
							<div class="b-content__path">

									<h2>Бизнес-центр "Вымпел"</h2>


									<div class="row">
										<div class="col-md-8">
											<div class="b-detailed-slider" id="lightgallery">
												<div>
													<a href="img/detailed/detailed.png" class="lightgallery-item">
												      <img src="img/detailed/detailed.png" width="995px" />
												  </a>
												</div>
												<div>
												  <a href="img/detailed/detailed.png" class="lightgallery-item">
												      <img src="img/detailed/detailed.png" width="995px" />
												  </a>
												</div>
												<div>
												  <a href="img/detailed/detailed.png" class="lightgallery-item">
												      <img src="img/detailed/detailed.png" width="995px" />
												  </a>
											 	</div>
											 	<div>
												  <a href="img/detailed/detailed.png" class="lightgallery-item">
												      <img src="img/detailed/detailed.png" width="995px" />
												  </a>
											  </div>
											</div>
										</div>
										<div class="col-md-4">
											<div class="b-detailed-info">
												<div class="b-detailed-info__path">
													<p>Ст. метро: <a href="#">Пролетарская</a></p>
													<p>Площадь: 12 кв.м</p>
												</div>
												<div class="b-detailed-info__path">
													<p><b>Ставка аренды за помещение в месяц:</b></p>
													<div class="b-price">
														32 000 ₽
													</div>
												</div>
											</div>
										</div>
									</div>

									<div class="b-detailed-props-wrap">
										<div class="row">
											<div class="col-md-4">
												<div class="b-detailed-props">
													<div class="b-detailed-props__title">Объект</div>
													<ul class="b-detailed-props__list">
														<li>
															<b>Тип объекта:</b> офис
														</li>
														<li>
															<b>Общая площадь:</b> 12 м2
														</li>
														<li>
															<b>Хар-ки:</b> административное здание , юр. адрес
														</li>
														<li>
															<b>Подходит под:</b> другое
														</li>
														<li>
															<b>Юридический адрес:</b> ИФНС №9
														</li>
													</ul>
												</div>
											</div>
											<div class="col-md-4">
												<div class="b-detailed-props">
													<div class="b-detailed-props__title">Местоположение</div>
													<ul class="b-detailed-props__list">
														<li>
															<b>Район:</b> ЮВАО
														</li>
														<li>
															<b>Метро:</b> м. Пролетарская
														</li>
														<li>
															<b>От метро:</b> 15 мин.пешком
														</li>
														<li>
															<b>Адрес:</b> Талалихина
														</li>
													</ul>
												</div>
											</div>
											<div class="col-md-4">
												<div class="b-detailed-props">
													<div>
														<a href="#" class="btn btn--red">Оставить заявку</a>
													</div>
													<a href="#">Прочитать отзывы</a>
												</div>
											</div>
										</div>
									</div>

							</div>

							<div class="b-content__path">
								<h3>О проекте</h3>
								<p>Бизнес-центр «Вымпел» – шестиэтажное офисное здание, общей площадью 51 297 кв.м, расположенное в районе Савёловский, Северного административного округа Москвы. В 15 минутах пешком от станций метро Динамо и Дмитровская. В бизнес-центре развитая инфраструктура: 1 и 2 этаж занимает торговая галерея, кафе, рестораны, отделения банков и предприятия сервисных услуг. Бизнес-центр «Вымпел» оснащен современными инженерными системами и отвечает всем международным требованиям, предъявляемым к офисным зданиям класса «В». Транспортную доступность обеспечивает близость Ленинградский проспект и Третье транспортное кольцо, для арендаторов предусмотрена наземная парковка. Бизнес-центр «Вымпел» расположен на территории обслуживания налоговой инспекции №14.</p>
							</div>

							<div class="b-content__path">
								<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d8982.52379237804!2d37.620331787942334!3d55.74754101880859!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x46b54afc73d4b0c9%3A0x3d44d6cc5757cf4c!2z0JzQvtGB0LrQstCwLCDQoNC-0YHRgdC40Y8!5e0!3m2!1sru!2sfr!4v1524736283593" width="100%" height="450px" frameborder="0" style="border:0;" allowfullscreen></iframe>
							</div>

							<div class="b-content__path">
								<div class="gray-block d-flex flex-column justify-content-between min-h-100 ">
									<h4 class="gray-block__title">Отзывы</h4>
									<div class="b-preview-review-wrap b-preview-review-wrap--bg">
											<div class="b-preview-review">
												<a href="#" class="b-preview-review__name">Ирина Антонова</a>
												<div class="b-preview-review__txt">
													<p>Приобретение коммерческой недвижимости в собственность не всегда оправдано экономически.</p>
												</div>
												<div class="b-preview-review__date">12 марта 2018</div>
											</div>
											<div class="b-preview-review">
												<a href="#" class="b-preview-review__name">Святослав Венгржановский</a>
												<div class="b-preview-review__txt">
													<p>Помещение под офис необходимо выбирать с учетом специфики компании, в соответствии со всеми предъявляемыми ему требованиями. В Москве возможна аренда коммерческой недвижимости с любыми параметрами в широком ценовом диапазоне — от недорогой до элитной.</p>
												</div>
												<div class="b-preview-review__date">12 марта 2018</div>
											</div>

									</div>
									<div class="txt-c">
										<a href="#" class="btn  btn--blue">Показать еще (245)</a>
									</div>
								</div>
							</div>

							<div class="b-content__path">
								<div class="white-block">
									<div class="white-block__title">Похожие товары</div>
									<div class="b-tab-wrapper">
										<div class="b-tabs">
											<span class="b-tabs__item active">По цене</span>
											<span class="b-tabs__item">По назначению</span>
											<span class="b-tabs__item">По расположению</span>        
										</div>
										<div class="b-tab-content">
											<div class="b-tab-content__item">
												<div class="b-products b-products--grid related-products-slider">
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
												</div>
											</div>
											<div class="b-tab-content__item">
												<div class="b-products b-products--grid related-products-slider">
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
												</div>
											</div>
											<div class="b-tab-content__item">
												<div class="b-products b-products--grid related-products-slider">
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
													<div class="b-catalog-item">
														<div class="b-catalog-item__body">
															<div href="#" class="b-catalog-item__img">
																<a href="#">
																	<img src="img/products/catalog-item-1.png" alt="img">
																</a>
																<div class="b-catalog-item__lot">Лот: <span>376842</span></div>
															</div>
															<div class="b-catalog-item__info">
																<div>		
																	<a href="#" class="b-catalog-item__name">Аренда офисов в бизнес-парке «Вымпел»</a>
																</div>
																<ul class="b-catalog-item__prop">
																	<li>
																		<p>Округ: ЦАО</p>
																	</li>
																	<li>
																		<p>Класс: В</p>
																	</li>
																	<li>
																		<div class="d-flex align-items-center">
																		Метро: 
																		<span class="metro-stations right-scroll">
																			<button class="metro-stations__scroll-l icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																			<span class="metro-stations__body">
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																				<span class="metro-stations__color" style="background-color: #7A22AC"></span>Таганская
																			</span>
																			<button class="metro-stations__scroll-r icons-hover">
																				<i class="icon icon-arrow-black icons-hover--visible"></i>
																				<i class="icon icon-arrow-red"></i>
																			</button>
																		</span>
																		</div>
																	</li>
																	<li>
																		<p>Площадь: 848 м²</p>
																	</li>
																</ul>
															</div>
														</div>
														<div class="b-catalog-item__result">
															<p>Ставка аренды за м<sup>2</sup>/год</p>
															<div class="b-catalog-item__price">
																32 000 ₽
															</div>
														</div>
													</div>
												</div>
											</div>
										</div>

									</div>
								</div>
							</div>

						</main>
						
						<? include('footer.php'); ?>

					</div>
				</div>
			</div>
		

	</div>

	<script src="js/scripts.min.js"></script>

</body>
</html>
