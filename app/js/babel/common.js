...
$(function () {
	...
	function productGridSlider() {
		if ($(window).width() < 1500) {
			if ($('.b-products--grid-slider').hasClass('slick-initialized')) return;
	...
	$('.metro-stations').each(function () {
		...
		function calcProp() {
			bodyWidth = Math.ceil($body.width()), maxScroll = $body[0].scrollWidth, scrollLeft = 0;
		}
	...
	function formatTemplateResult(state) {
		if (!state.id) {
			try {
				var $state = $('<span class="select2-selection__choice__span-text" style="' + state.element.attributes.style.value + '">' + state.text + '</span>');
				return $state;
			} catch (err) {
				var $state = $('<span class="select2-selection__choice__span-text">' + state.text + '</span>');
				return $state;
			}
		} else {
	...
	$('.b-review').each(function () {
		if ($(this).find('.b-review__text').height() > 80) $(this).addClass('b-review--close');
	});
	...
	$('.b-filter-region-header input').on("change", function () {
		...
		$('.b-filter-region-global-list > li').hide().each(function () {
			if ($(this).attr('data-region-list') === value || value == "") $(this).show();
		});
	});
...
$(document).ready(function () {
	...
	if ($('#map').length) {
		var drawLineOverMap = function drawLineOverMap(map) {
